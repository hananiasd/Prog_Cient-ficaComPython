...
print('\n\nQuestão 02')
temp = 0
lista_aux = monthly_mean.copy()
for A in lista_aux:
	aux = A
	if aux > temp :
		temp = aux
		indice = lista_aux.index(temp) #descobrir o indice do mes com maior incidência de luz do sol
mes = month_names[indice]

print('Mês que teve maior incidência de luz no período.')
print('%s: %.1f' %(mes, temp))
...
for i in range(1930, 2010, 10):
	anos.insert(len(anos), i)

for i in range(0, 8):
	decada_anos = []
	for k in range(0, 10):
		decada_anos.insert(len(decada_anos), data[0])
		data.pop(0)
	decada_valor[str(anos[i]) + '-' + str(anos[i]+9)] = decada_anos

media_dia = {}
for decada in decada_valor:
	janeiro = 0
	dezembro = 0
	for ano in range(0, 10):
		janeiro += decada_valor[decada][ano][0]/31
		dezembro += decada_valor[decada][ano][11]/31
